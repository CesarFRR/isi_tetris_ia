...
class GridManager:
    ...
    def compute_piece(self, piece:Piece):
        ...
        for i in range(len(h_list)):
            current = sum(self.grid.calculate_heuristics(piece, i))
            h_list[i]= current
    ...
    def compute_piece_all_rotations(self, piece:Piece):
        ...
        for i in range(len(h_list_all)):
            ...
            h_list_all[i]=self.compute_piece(piece)
    ...
    def get_best_choice(self, piece:Piece):
        ...
        max_index, max_value = max(enumerate(self.compute_piece_all_rotations(piece)), key=lambda x: x[1][0])
    ...
    def place_piece(self, piece:Piece, column:int, rotation:int):
        ...
        self.grid = self.grid.place_piece_final(piece, column, rotation)
